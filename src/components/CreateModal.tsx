...
export const CreateLot = () => {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setIsOpen}>
      <Dialog.Trigger className="rounded border-2 border-violet-500 bg-violet-500 px-2 py-1 text-white shadow hover:text-slate-200">
        Create new lot
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/50">
          <Dialog.Content className="fixed left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded bg-white p-8 text-gray-900 shadow">
            <div className="flex items-center justify-between">
              <Dialog.Title className="text-xl text-gray-800">
                Create a new lot
              </Dialog.Title>
